...
export function buildCarteraWorkbook(data: CarteraExportData) {
  ...
  function createSheet(name: string, client: string, nit: string, headers: string[], rows: Value[][], widths: number[], numericCols: number[], totals?: Value[], cards?: [number,number,number,string]) {
    ...
    ws['!rows']=values.map((row,r)=>({hpt:r===0?45:r===4?38:r===10?42:r===cardRow+1?42:r<10?23:Math.min(409,Math.max(30,...row.map((v,c)=>String(v).split('\n').reduce((n,line)=>n+Math.max(1,Math.ceil(line.length/(widths[c]||30))),0)*15+14)))}));
    ...
    for(let r=0;r<values.length;r++)for(let c=0;c<count;c++){
      ...
      cell.s={font:{name:'Calibri',sz:r===0?28:r===4?25:r===cardRow+1?20:11,bold:banner||head||total||r===4||r===6||r===cardRow+1||r===supportRow,color:{rgb:banner||head?'FFFFFF':r===4?BLUE:INK}},
        ...
        alignment:{vertical:'center',horizontal:body&&numericCols.includes(c)?'right':'left',wrapText:true},
        border:body||head||total?{bottom:{style:'thin',color:{rgb:'D7E1EC'}}}:{}};
  ...
  if(payments.length)createSheet('Abonos',data.vista==='cliente'?data.clientName:'Todos los clientes',data.vista==='cliente'?data.clientNit:'',['Cliente','Documento','Fecha de abono','Valor aplicado','Medio de pago','Referencia','Observaciones'],payments,[35,28,24,24,24,32,65],[3]);
  return workbook;
}
